...
module.exports =  (passport) => {
  ...
  passport.deserializeUser((sessionUser, done) => {
    ...
    // The sessionUser object is different =require(the user mongoose
    // collection

    // It is actually req.session.passport.user and comes =require(the
    // session collection
    done(null, sessionUser);
  });
  ...
  (req, username, password, done) => {
    ...
    if(!checkLength(username, bounds.username.minLength, bounds.username.maxLength)) {
      ...
      return done(null,
        ...
        { signupMessage : 'Invalid username length. It should be 3-16 length.' }
      );
    }
